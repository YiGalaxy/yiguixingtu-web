...
>
> **214 个测试用例 + ESLint + 生产构建，全部在 CI 里自动跑**（见下文「持续集成」）
...
│   ├── composables
│   │   ├── useApi.ts                # 统一请求封装：带 token、判 body.code、401/403/429 处理
...
│   │   └── apiError.ts              # 两种 429（限流 / 重复提交）的判断与文案、追踪号 X-Trace-Id
│   ├── middleware
...
| 运行测试 | `npm run test` | 214 个用例；**不需要后端与数据库**，CI 里不用起任何服务 |
...
| 测试文件 | 用例数 | 覆盖 |
|---------|:---:|------|
| `test/useApi.nuxt.spec.ts` | 30 | 请求封装的**六条失败分支**（业务 code≠200 / 幂等重复提交 / 401 / 403 / **429 限流** / 网络异常）、**两种 429 的提示必须不同**、429 不清 token、token 是否带上、baseURL 与参数透传、`resolveApiBase` 服务端与浏览器两种地址；以及**追踪号**：业务失败（靠 ofetch 的 `onResponse` 钩子拿响应头）与 HTTP 429/500 都要带上 `X-Trace-Id`、没有这个头时**不出现"追踪号"三个字也不出现空括号**、成功时一条提示都不弹 |
