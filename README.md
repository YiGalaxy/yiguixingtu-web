...
>
> **205 个测试用例 + ESLint + 生产构建，全部在 CI 里自动跑**（见下文「持续集成」）
...
│   │   └── apiError.ts              # 两种 429（限流 / 重复提交）的判断与文案
│   ├── middleware
...
| 运行测试 | `npm run test` | 205 个用例；**不需要后端与数据库**，CI 里不用起任何服务 |
...
| 测试文件 | 用例数 | 覆盖 |
|---------|:---:|------|
| `test/useApi.nuxt.spec.ts` | 23 | 请求封装的**六条失败分支**（业务 code≠200 / 幂等重复提交 / 401 / 403 / **429 限流** / 网络异常）、**两种 429 的提示必须不同**、429 不清 token、token 是否带上、baseURL 与参数透传、`resolveApiBase` 服务端与浏览器两种地址 |
| `test/useAuth.nuxt.spec.ts` | 15 | 登出链路：**会调用后端** `POST /auth/logout`、带上当前 token、清空 `token` 与 `user`、接口失败也照样清本地、不再跳到不存在的 `/login`；以及**登录被限流**：认得出 HTTP 429、提示里要有"频繁"、没有 body 时用兜底文案、冷却是 10 秒且逐秒递减、冷却期间**不再发请求**、冷却结束后能正常登录、连点两下（一次限流一次成功）以成功为准；密码错误与网络异常都**不能**被误判成限流 |
