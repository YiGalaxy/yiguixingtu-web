...
>
> **515 个测试用例 + ESLint + 生产构建，全部在 CI 里自动跑**（见下文「持续集成」）
...
| 运行测试 | `npm run test` | 515 个用例；**不需要后端与数据库**，CI 里不用起任何服务 |
...
| `test/backgroundMusic.nuxt.spec.ts` | 6 | **背景音乐的一份状态、三个界面**（页脚开关 / ⚙ 设置面板 / 首页音乐卡片）。守的是"两处各写一份状态于是互相打架"这类 bug：**同时挂载外壳与首页**，点卡片上的播放键 → 页脚标签要变成"关闭背景音乐"；再从页脚关掉 → 卡片按钮要回到 ▶。另外：默认关闭且不往 localStorage 写东西；`setEnabled`/`toggle` 会写 `bg-music-enabled`（`1`/`0`）；`loadPreference` 能读回来；**`localStorage` 抛异常时（隐私模式）读退回默认值、写也不抛** —— 背景音乐不该有能力让整个站点打不开；以及"开关音乐**不会**去动背景视频的静音"（老实现是把它取消静音，那既不是音乐、视频音轨还会盖在音乐上） |
