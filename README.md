...
>
> **332 个测试用例 + ESLint + 生产构建，全部在 CI 里自动跑**（见下文「持续集成」）
...
- **前台**：首页文章信息流（**服务端渲染** + 分页 + 关键词防抖搜索 + 分类筛选 + **标签筛选**，三个筛选条件都同步到地址栏）、
  文章详情（Markdown 渲染 + **文章标签**，同样是服务端渲染），个人卡片的文章数 / 浏览量 / 分类数来自站点统计接口
...
│   │   └── article/[id].vue         # 文章详情（SSR + Markdown 渲染 + 标签 + 软 404）
│   ├── composables
...
| 运行测试 | `npm run test` | 332 个用例；**不需要后端与数据库**，CI 里不用起任何服务 |
...
| 文章详情 | `/article/:id` | Markdown 正文渲染（SSR）、**文章标签**（点了回首页按标签筛），已发布文章可访问 | 否 |
...
- **端到端（Playwright）测试还没有**：目前是组合式函数单测 + 六个页面组件测试
  （首页、首页内容真实性、文章详情页的标签、后台概览、后台文章弹窗的标签、后台标签管理；组件测试覆盖了"页面把逻辑接上去了没有"，
  但跨页面跳转、真实后端联调还测不到）
