...
>
> **479 个测试用例 + ESLint + 生产构建，全部在 CI 里自动跑**（见下文「持续集成」）
...
| 运行测试 | `npm run test` | 479 个用例；**不需要后端与数据库**，CI 里不用起任何服务 |
...
| `test/adminComments.nuxt.spec.ts` | 21 | 后台**评论审核**：默认按「待审核」筛选（`status=0` 真的发给了后端）、列表显示昵称 / 内容 / 文章标题 / 时间 / 邮箱 / IP、内容按**纯文本**渲染（`v-html` 在这里同样是 XSS 入口）、接口失败或返回非数组时不崩、切到「已通过」会带 `status=1` 重新查询（**真实交互**：点开下拉框再点选项）、选「全部」**不发 status 参数**（也不能把界面哨兵值 -1 发给后端）；**审核**：通过/拒绝走 `PUT /admin/comment/{id}/status`，且状态必须是**查询参数**而不是请求体（后端是 `@RequestParam`，放 body 里会 400 —— 这条断言就是钉它的）、成功后刷新列表、已是当前状态的按钮禁用；**角标**：进后台就显示待审核数量、为 0 时整个角标不渲染、在「已通过」里拒绝一条时会**额外查一次**待审核数（列表的 total 不代表待办数）；**删除**：二次确认里说明"前台也看不到了、无法恢复"、点取消不发请求、成功后那条消失；**404 自愈**：审核与删除撞上 404 都提示"已经不在了 / 列表已刷新"并真的重拉；文章被删除时"文章"列显示占位说明而不是空白 |
