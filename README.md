...
>
> **384 个测试用例 + ESLint + 生产构建，全部在 CI 里自动跑**（见下文「持续集成」）
...
- **后台**：登录后进入 `/admin`，可管理用户（分页 / 启用禁用 / 改角色 / 重置密码 / 删除）、
  文章（Markdown 编辑器 / 草稿与发布 / 下架 / 删除 / **多选标签**）与
...
│   │   ├── admin.vue                # 后台：用户 / 文章（+标签多选）/ 标签管理 / 只读分类 + Markdown 编辑器
...
| 运行测试 | `npm run test` | 384 个用例；**不需要后端与数据库**，CI 里不用起任何服务 |
...
| 后台 | `/admin` | 用户管理 + 文章管理（文章可多选标签）+ **标签管理**（增删改）+ **只读分类列表**；概览显示全站汇总数字（进入页面即主动加载）；head 带 `noindex` | **是（ADMIN）** |

后台的左侧菜单有六项：概览 / 文章管理 / 用户管理 / 标签管理 / 分类 / 设置。
其中「分类」目前是**只读**的（后端其实已经有分类的增删改接口，是前端这一批没接），
「设置」仍然是占位 —— 详见下文「后台的标签管理与只读分类」。
...
**站点统计为什么必须走后端接口（`useSiteStats.ts`）**首页个人卡片与后台概览的数字都来自 `GET /article/stats`，而不是前端自己算：
...
- **评论发布后要靠人工审核，目前后台还没有审核界面**：这一批只做了前台评论区，
  审核界面是下一步（在那之前，只能靠后端接口或 SQL 把 `status` 改成 1）。
  代价很实在：**站长不点通过，评论就一直不对外可见** —— 所以前台的提示写的是
  "等待审核"而不是"已发布"
...
- **端到端（Playwright）测试还没有**：目前是组合式函数单测 + 七个页面组件测试
  （首页、首页内容真实性、文章详情页的标签、文章详情页的评论区、后台概览、后台文章弹窗的标签、后台标签管理；组件测试覆盖了"页面把逻辑接上去了没有"，
  但跨页面跳转、真实后端联调还测不到）
...
- `app/pages/admin.vue` 单文件 **1545 行**，用户表格 / 文章表格 / 标签管理 / 编辑器弹窗都挤在一个文件里
  （标签这一批又长了三百多行，评论审核还要继续加 ——
