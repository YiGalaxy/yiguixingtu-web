...
>
> **316 个测试用例 + ESLint + 生产构建，全部在 CI 里自动跑**（见下文「持续集成」）
...
- **后台**：登录后进入 `/admin`，可管理用户（分页 / 启用禁用 / 改角色 / 重置密码 / 删除）
  与文章（Markdown 编辑器 / 草稿与发布 / 下架 / 删除 / **多选标签**），概览页显示全站汇总数字
...
│   │   ├── admin.vue                # 后台：用户管理 + 文章管理（含标签多选）+ Markdown 编辑器（head 带 noindex）
...
| 运行测试 | `npm run test` | 316 个用例；**不需要后端与数据库**，CI 里不用起任何服务 |
...
| 后台 | `/admin` | 用户管理 + 文章管理（文章可多选标签）；概览显示全站汇总数字（进入页面即主动加载）；head 带 `noindex` | **是（ADMIN）** |
...
- **端到端（Playwright）测试还没有**：目前是组合式函数单测 + 五个页面组件测试
  （首页、首页内容真实性、文章详情页的标签、后台概览、后台文章弹窗的标签；组件测试覆盖了"页面把逻辑接上去了没有"，
  但跨页面跳转、真实后端联调还测不到）
...
- `app/pages/admin.vue` 单文件 **1278 行**，用户表格 / 文章表格 / 编辑器弹窗都挤在一个文件里
  （加了标签多选之后又长了一截；下面还在继续接标签管理与评论审核 ——
  "按模块拆成子组件"该提上日程了。顺带说明：这一行以前写的是 1001 行，
  那个数字本身早就过期了，这次按实际重新数过）
