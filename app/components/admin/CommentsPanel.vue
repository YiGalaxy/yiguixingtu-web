<template>
  ...
  <div class="panel glass">
    <!-- 列宽说明：可伸缩的三列（内容 / 文章标题 / 邮箱）都开了
         ...
         ⚠️ 这两段加起来 ≥ 1162px，比常见的可用宽度宽 ⇒ 窄一点就要横向滚动。
         这是**可以接受的**：2026-09-11 起所有后台表格都不再用固定列，
         整张表一起滚动、滚动条常显（一眼看出右边还有内容），
         「操作」列与其它列完全同款 —— 不会再有"字叠字"。
         （这一列前后踩过两次坑：固定列没底色时下层文字透上来；补上底色之后
           那一列又和别的列长得不一样。两版补救都不如"干脆不做特殊对待"。）
