...
/* =====================================================================
   关于「操作」列：**所有后台表格都不再用固定列**（2026-09-11 定的）
   =====================================================================
   原来 9 个面板的「操作」列都写了 `fixed="right"`，想让它在横向滚动时钉在右边。
   代价是：Element Plus 的固定列是 `position: sticky`，而本项目的暗色主题把表格
   背景设成了 `transparent`（好让毛玻璃面板透出来）⇒ 它**没有自己的底色**，
   横向滚动的内容会从它下面透上来（"字叠字"）。
   为此先后试过两版补救：
     · 第一版：给固定列补一层半透明底 + 模糊，包在 `@media (max-width: 900px)` 里 ——
       而"桌面宽度下装得下"这个前提会**随列数变化而失效**（评论表加了两列之后
       列宽合计涨到 1162px），于是 900~1460px 这一段里固定列正在压着「IP」列、
       媒体查询却还没生效 ⇒ 用户在后台看到"IP 和操作挤在一起"
     · 第二版：把补底色改成无条件生效 —— 这确实挡住了穿透，但**那一列从此和别的列
       长得不一样**（一层明显的深色块），用户随即反馈"操作那一列不是和前面一样的毛玻璃，
       而是有颜色的" ⇒ 说明这个方向本身就不对
   ⇒ 结论：**不再给这一列任何特殊对待**。整张表一起横向滚动（滚动条常显，见下），
      「操作」列与其他列完全同款（同样的透明底、同样的毛玻璃），不再有压住谁的问题。
   契约测试盯着这件事（`test/adminTableLayout.spec.ts`：9 个面板里不许再出现固定列）。 */

/* 表格的横向滚动条改成**常显**（el-scrollbar 默认藏在悬停之后）。
   【为什么必须改】后台这些表格的列宽合计基本都大于容器（评论表 1162px、文章表更长），
   也就是"横向能滚"是常态 —— 而它现在是**唯一的**触达右侧列（含「操作」）的方式。
   滚动条默认不显示 ⇒ 用户看不见右边还有内容，只会把排版当成坏了。
