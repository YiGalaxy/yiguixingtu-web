...
//  评论管理（审核 / 删除）
//
//  【这个页面的存在意义】评论默认是【待审核】的，不点通过前台就一直看不到它。
//   也就是说：没有这个页面，别人写的评论永远只有作者自己看得见 ——
//   前台那句"等待审核"就成了一句空话。所以这不是"锦上添花的后台功能"，
//   而是评论模块能不能真正跑起来的最后一环。
//
//  【为什么状态筛选的默认值是「待审核」而不是「全部」】
//   这个页面 90% 的用途是"把攒下的待审核处理掉"。
//   默认「全部」的话，新评论会淹没在已经处理过的评论里，
//   每进一次页面都要先手动选一次筛选。
// ================================================================
 ...
 * 而这里的"全部"其实是"不按状态过滤"。
 * 【为什么不用 null 表示】el-option 的 value 类型不含 Null，传 null 会每次渲染
 * 打一条 prop 类型警告（见模板里那段注释），所以用 -1 这个不可能是真实状态的数字。
 */
...
/** 待审核总数：菜单上的角标用它。它和 cmTotal 不是一回事 ——
 *  cmTotal 是"当前筛选条件下的条数"，角标要的是"待审核有多少条" */
...
const fetchComments = async () => {
  ...
  if (cmQuery.status === commentStatus.PENDING) pendingCommentCount.value = cmTotal.value
}

/** 单独查一次"待审核有多少条"（当前视图不是待审核时才需要） */
const fetchPendingCommentCount = async () => {
  // size=1：只要 total 这个总数，不需要真的把那一页数据拉回来
  const res = await request('/admin/comment/page', { params: { page: 1, size: 1, status: commentStatus.PENDING } })
...
const refreshComments = async () => {
  await fetchComments()
  if (cmQuery.status !== commentStatus.PENDING) await fetchPendingCommentCount()
}
...
onMounted(async () => {
  ...
  await ensureUser()
  await Promise.all([
    fetchUsers(), fetchCategories(), fetchTags(), fetchArticles(), loadOverview(),
